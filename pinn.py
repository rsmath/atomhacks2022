...
EPSILON = 2.2e-8 if PRECISION == torch.float32 else 2.2e-16
        ...
        for i in range(1, epochs + 1):
            try:
                def closure():
                    ...
                    if self.supervised:
                        print('Supervised enabled')
                        l1 = torch.mean(torch.square(torch.flatten(self.u_tilde) - torch.flatten(u_pred_tilde)))
                ...
                self.logged_results['training_mse1'] = self.mse1
                self.logged_results['test_mse1'] = self.test_mse1
                self.logged_results['training_u1'] = self.u1_error
                self.logged_results['test_u1'] = self.test_u1_error
...
if __name__ == "__main__":
    ...
    for cur_supervised_option in supervised_options:
        ...
        for size in training_size:
            ...
            file_name = f"{size}"
            test_name = f"21748_test"

            print('\n\n')
            print('+'*70)
            print(f'Running Approach 1 on {file_name} with {activation_function}')
            print('+'*70)
            print('\n\n')
